...
* -kmer-size : the k-mer size [default '31']. By default, the largest kmer-size allowed is 128. To use k>128, you will need to re-compile TakeABreak with the two commands in the build directory: 'cmake -DKSIZE_LIST="32 64 96 256" ..' and then 'make'. To go back to default, replace 256 by 128. Note that increasing the range between two consecutive kmer-sizes in the list can have an impact on the size of the output h5 files (but none on the results).
...
###Example from raw input reads:
    ...
    ./TakeABreak -in data/toy_example_reads.fasta,data/toy_example_with_inv_reads.fasta \
